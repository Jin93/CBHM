###################### an example code for CBHM using H distance. 
...
for (sim in 1:num.sim)
{
  ...
  jags.fit <- jags.model(file = "cbhm_h.txt",data = jags.data,
...
Q.cbhm.h=quantile(posterior.cbhm.b,1-alpha) # probability cut-off for the final decision
...
if (length(stage2.cont)>0)
{
  ...
  decision.sbhm[sim,stage2.cont]=ifelse(posterior>Q.sbhm.h,1,0)
}
